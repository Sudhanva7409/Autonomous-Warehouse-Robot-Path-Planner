...
/**
 * A* pathfinding algorithm implementation for warehouse navigation.
 * Uses heuristic-based search to find optimal paths efficiently.
 */
public class AStar {
    ...
    // Movement directions: up, down, left, right (4-directional)
    ...
    // Optional: Add diagonal movement (8-directional)
    ...
    /**
     * Creates a new A* pathfinder for the given grid.
     * 
    ...
    public List<Node> findPath(Node start, Node goal) {
        ...
        // Create fresh start node with scores
        Node startNode = getOrCreateNode(start.getX(), start.getY());
        startNode.setGScore(0);
        startNode.setHScore(calculateHeuristic(startNode, goal));
        
        openSet.add(startNode);
        
        System.out.println("\n[A*] Starting pathfinding...");
        ...
        while (!openSet.isEmpty()) {
            ...
            for (Node neighbor : getNeighbors(current)) {
                ...
                if (tentativeGScore < neighbor.getGScore()) {
                    ...
                    neighbor.setHScore(calculateHeuristic(neighbor, goal));
        ...
        for (int[] dir : directions) {
            ...
            if (grid.isWalkable(newX, newY)) {
                ...
                if (allowDiagonal && Math.abs(dir[0]) == 1 && Math.abs(dir[1]) == 1) {
                    // Check adjacent cells to prevent cutting corners
    ...
    /**
     * Reconstructs the path from goal to start by following parent links.
     * 
    ...
    public void printStatistics() {
        System.out.println("\n[A* Statistics]");
        System.out.println("  Algorithm: A* (A-Star)");
        System.out.println("  Movement: " + (allowDiagonal ? "8-directional" : "4-directional"));
        System.out.println("  Nodes explored: " + nodesExplored);
        System.out.println("  Heuristic: " + (allowDiagonal ? "Euclidean" : "Manhattan"));
    }
}